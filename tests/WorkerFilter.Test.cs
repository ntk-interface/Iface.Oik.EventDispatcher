...
namespace Iface.Oik.EventDispatcher.Test
{
    public class WorkerFilterTest
    {
        public class Constructor
        {
            ...
            [Theory]
            [InlineData(TmEventTypes.Control, TmEventTypes.Control)]
            [InlineData(
                TmEventTypes.Control | TmEventTypes.Alarm,
                TmEventTypes.Control,
                TmEventTypes.Alarm
            )]
            [InlineData(
                TmEventTypes.Control | TmEventTypes.Alarm | TmEventTypes.Acknowledge,
                TmEventTypes.Control,
                TmEventTypes.Alarm,
            ...
            {
                var filter = new WorkerFilter(
                    new WorkerFilterConfig { Importances = importances.ToList() }
                );
            ...
            {
                var filter = new WorkerFilter(
                    new WorkerFilterConfig { Statuses = new List<string>() }
                );
            ...
            {
                var filter = new WorkerFilter(
                    new WorkerFilterConfig { Analogs = new List<string>() }
                );
        ...
        public class IsEventSuitableMethod
        {
            ...
            public void ReturnsTrueWhenEmptyFilter()
            {
                ...
                {
                    filter.IsEventSuitable(tmEvent).Should().BeTrue();
                }
            }
            ...
            public void ReturnsCorrectValueWithImportancesFilter(int importance, bool expected)
            {
                ...
                var tmEvent = TmEventUtil.CreateRandomValidTmEvent(dto =>
                    dto.Importance = (short)importance
                );
            ...
            public void ReturnsCorrectValueWithStatusesFilter(
                int ch,
                int rtu,
                int point,
                bool expected
            ...
            public void ReturnsFalseWithStatusesAnalogsFilterAndZeroAddr()
            {
                ...
                var analogs = new List<string> { "20:3:2..4", "20:3:6" };
                var filter = new WorkerFilter(
                    new WorkerFilterConfig { Statuses = statuses, Analogs = analogs }
                );
                var tmEvent = TmEventUtil.CreateRandomValidTmEvent(dto =>
                {
                    dto.TmType = 0;
