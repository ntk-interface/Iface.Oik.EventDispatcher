...
public class WorkerFilter
{
    ...
    {
        if (Statuses.Count == 0 && Analogs.Count == 0)
        {
            return true;
        }
        return (tmEvent.HasTmStatus && Statuses.BinarySearch(tmEvent.TmAddrTma) >= 0)
            || (tmEvent.HasTmAnalog && Analogs.BinarySearch(tmEvent.TmAddrTma) >= 0);
    }
}
